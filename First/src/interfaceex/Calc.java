...
public interface Calc {
	...
	default void description() {
		...
		*/
		...
		/*
		 * private 메서드
		 * 자바 9부터는 인터페이스에 private 메서드 구현이 가능
		 * private 메서드는 인터페이스를 구현한 클래스에서 사용/재정의할 수 없음
		 * 따라서 기존에 구현된 코드를 변경하지 않고 인터페이스를 구현한 클래스에서 공통으로 사용할 때 private 메서드로 구현 시 코드 재사용성을 높일 수 잉씀
		 * 또한 클라이언트 프로그램에 제공할 기본 기능을 private 메서드로 구현하기도 함
		 * 
		 * private 메서드는 코드를 모두 구현해야하므로 추상메서드에는 사용할 수 없음
		 * static 예약어는 함께 쓸 수 있으며 정적 메서드에서 호출해 사용함
		*/
	}
